...
import { DiscoveryState } from "../../../../src/discovery/models/DiscoveryState";

// @todo The DiscoverAccounts command should probably use `Account`, not `AccountDTO`.
...
describe("discovery/DiscoverAccounts", () => {
  ...
  describe("test on onModuleInit()", () => {
    it("should have correct flow and initialized result", async () => {
      ...
      const discoverCall = jest.spyOn(service, "discover").mockReturnThis();
  ...
  describe("test on syncState()", () => {
    it("should have correct flow and result", async () => {
      ...
      expect(findOneCall).toHaveBeenCalledWith(new StateQuery(undefined, "discovery:DiscoverAccounts"));
