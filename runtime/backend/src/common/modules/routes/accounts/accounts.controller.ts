...
export class AccountsController {
  ...
  @ApiExtraModels(Account, PaginatedResutDto)
  @ApiOkResponse({
    schema: {
      allOf: [
        { $ref: getSchemaPath(PaginatedResutDto) },
        {
          properties: {
            data: {
              type: 'array',
  ...
  find(@Query() query: AccountQueryDTO): Promise<PaginatedResutDto<Account>> {
    return this.accountsService.find(query);
  }
}
