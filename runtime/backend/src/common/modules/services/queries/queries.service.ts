...
type MongoQueryConditionValue = boolean | number | string | any[] | MongoQueryRoutine;
...
export class QueriesService {
   ...
   * @access public
   * @async
   * @param   {PagePaginatedQueryDto} query     The query configuration with `sort`, `order`, `pageNumber`, `pageSize`.
   * @param   {Model<any>}            model     The model *class instance* used for resulting documents.
   * @returns {Promise<PaginatedResutDto<any>>} The matching documents after execution of the query.
   */
  public async find(
    ...
    const { queryCursor, querySorter, searchQuery } = this.getQueryConfig(query);
   ...
   */
  protected getQueryConfig(query: PagePaginatedQueryDto): SearchQuery {
   ...
   */
  protected sanitizeSearchQuery(searchQuery: UnsafeQueryConditions): MongoQueryConditions {
    ...
    // (3) array
      ...
      const newValues = value.map((item) => {
        ...
        if (Object.keys(sanitizedItem).includes("$in")) {
          ...
          const val: number[] | string[] = (sanitizedItem as MongoQueryRoutine)["$in"];
