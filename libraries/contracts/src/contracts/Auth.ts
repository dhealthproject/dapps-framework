 ...
 * objects passed as **inputs** to {@link Auth} instances.
 * <br /><br />
...
export interface AuthParameters extends ContractParameters {
   ...
   * <br /><br />
   * Note that the referral code is *optional* in the {@link Auth}
   * contract and may be omitted. If you require the presence
   * of this field, it is recommended to use the {@link Referral}
   * contract instead.
   *
   * @access public
   * @example `"ELEVATE2022"`
 ...
 * *authentication* contract which is used to discover operations
 * of log-in in the dApp.
 * <br /><br />
 * Use this class whenever you are interpreting an operation of
 * log-in for your dApp. Also, an instance of this class is created
 * using {@link Factory.createFromJSON} given a contract signature
 * that matches: `/:auth$/` (ends with).
 * <br /><br />
 * #### Parameters
 * Following *inputs* apply to the {@link Auth} contract class:
 * | Input | Type | Required? | Description |
 ...
 * ```ts
 * // creating an authentication contract
 * const contract: Auth = new Auth({
...
export class Auth extends Contract {
   ...
   *
   * @access public
   * @param   {AuthParameters}      inputs      The inputs that are used during execution ("arguments").
   ...
   * and optionally include the `refCode` field if it is non-empty.
   * <br /><br />
  ...
  public get body(): ObjectLiteral {
    // authentication code is obligatory
    const fields: ObjectLiteral = {
      challenge: this.inputs.challenge,
    };

    // referral code is optional
    if (undefined !== this.inputs.refCode && this.inputs.refCode.length > 0) {
      fields["refCode"] = this.inputs.refCode;
    }
