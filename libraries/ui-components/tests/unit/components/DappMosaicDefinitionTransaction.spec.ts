...
describe("DappMosaicDefinitionTransaction -->", () => {
  ...
  it("should display correct elements with correct attributes & classes", () => {
    ...
    const dappAccountAvatarElement = svgElement.findComponent(DappAccountAvatar);
    const dappMosaicIconElement = svgElement.findComponent(DappMosaicIcon);
    const dappTransactionArrowElement = svgElement.findComponent(DappTransactionArrow);
    ...
    expect(svgElement.attributes()["xmlns:xlink"]).to.equals("http://www.w3.org/1999/xlink");
    ...
    expect(dappAccountAvatarElement.attributes().address).to.equals("NDXB7NNXCHXEMQRUSCDGKDYZB2X5EDUM5GDDOMY");
    ...
    expect(dappMosaicIconElement.attributes().mosaic).to.deep.equals("[object Object]");
    ...
    expect(dappAddCircleElement.attributes().content).to.equals("[object Object]");
    expect(dappAddCircleElement.attributes().title).to.equals("Mosaic Definition");
    ...
    expect(textElement.classes()).to.include("dappMosaicDefinitionTransaction-message");
