...
describe("DappTransferTransaction -->", () => {
  ...
  it("should display correct elements", () => {
    ...
    const dappAccountAvatarEls = svgEl.findAllComponents(DappAccountAvatar);
    const dappTransactionArrowEl = svgEl.findComponent(DappTransactionArrow);
    const dappMessageCircleEl = svgEl.findComponent(DappMessageCircle);
    const dappMosaicCircleEl = svgEl.findComponent(DappMosaicCircle);
    const textEl = svgEl.find("text");
    const titleEl = textEl.find("title");
    expect(divEl.exists()).to.be.true;
    expect(svgEl.exists()).to.be.true;
    expect(dappAccountAvatarEls.length).to.equals(2);
    expect(dappTransactionArrowEl.exists()).to.be.true;
    expect(dappMessageCircleEl.exists()).to.be.true;
    expect(dappMosaicCircleEl.exists()).to.be.true;
    expect(textEl.exists()).to.be.true;
